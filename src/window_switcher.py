"""Window enumeration and switching on macOS.

PyObjC when it is available, AppleScript via subprocess when it is not.
"""
...
# There is deliberately no module-level "known apps" table here. One used to
# exist, filled from config["known_apps"] at startup -- and never read by
# anything. Which windows get cycled is decided by WindowCycler.app_names, set
# from config["selected_apps"], and that is the only place it is decided.


class WindowInfo(NamedTuple):
    hwnd: int         # Owner PID (Quartz path) or index (AppleScript path)
    title: str
...
class WindowCycler:
    ...
    def __init__(self, app_names: list[str] | None = None) -> None:
        self._app_names: list[str] = app_names or ["Code"]
