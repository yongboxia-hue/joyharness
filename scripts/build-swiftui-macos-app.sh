...
VERSION="${JOYHARNESS_VERSION:-0.2.3}"
...
SLICES=()
# The compiler writes out every string the interface can show, exactly as it
# will be looked up (interpolations already turned into %@ / %lld). One slice
# is enough; verify-swiftui-macos-app.sh checks each against the English table.
LOC_STRINGS_DIR="$ROOT_DIR/build/localized-strings"
rm -rf "$LOC_STRINGS_DIR" && mkdir -p "$LOC_STRINGS_DIR"
LOC_FLAGS=(-emit-localized-strings -emit-localized-strings-path "$LOC_STRINGS_DIR")
...
# Chinese is the source text, so its table maps every key to itself. It has to
# exist all the same: with no zh-Hans table, lookup falls through to the
# development region and a Chinese Mac shows English. Written from the
# compiler's own key list, so it can never be one string behind.
python3 - "$LOC_STRINGS_DIR" "$RESOURCE_DIR/zh-Hans.lproj/Localizable.strings" <<'PY'
import glob, json, sys
keys = []
for path in sorted(glob.glob(sys.argv[1] + "/*.stringsdata")):
    for entry in json.load(open(path, encoding="utf-8"))["tables"].get("Localizable", []):
        if entry["key"] not in keys:
            keys.append(entry["key"])
def quote(text):
    return '"' + text.replace("\\", "\\\\").replace('"', '\\"').replace("\n", "\\n") + '"'
with open(sys.argv[2], "w", encoding="utf-8") as table:
    table.write("/* Generated by build-swiftui-macos-app.sh: the source text is Chinese. */\n")
    for key in keys:
        table.write(f"{quote(key)} = {quote(key)};\n")
PY
plutil -lint "$RESOURCE_DIR/zh-Hans.lproj/Localizable.strings" >/dev/null
...
<plist version="1.0">
<dict>
  <!-- English is the fallback for every language the app does not speak;
       Chinese users get Chinese because zh-Hans is listed below. The per-app
       language menu in System Settings offers exactly these two. -->
